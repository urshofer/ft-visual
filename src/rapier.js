...
async function RapierPhysics() {
  ...
  function addGroup(group, mass = 0, restitution = 0, friction = 0.55) {
    ...
    const { body, collider } = createBody(
      group.position,
      group.quaternion,
      group.userData.physics.mass || mass,
      shape,
    );
  ...
  const MAX_TIMESTEP = 1 / 30; // clamp physics dt to prevent explosions after tab switch
  let paused = false;

  // Pause physics when tab is hidden, reset timer on return
  document.addEventListener("visibilitychange", () => {
    if (document.hidden) {
      paused = true;
    } else {
      paused = false;
      timer.update(); // consume the accumulated time so getDelta() is near-zero
    }
  });
  ...
  function step() {
    ...
    world.timestep = Math.min(timer.getDelta(), MAX_TIMESTEP);
    world.step();
