...
for (let x = 0; x < W; x++) drip[x] = 0.5 + Math.sin(x * .37 + 1.1) * .25 + Math.sin(x * .89 + 2.9) * .20;
...
function hsl(h, s, l) {
  ...
  const i = (h * 6 | 0) % 6;
  return [[c, x2, 0], [x2, c, 0], [0, c, x2], [0, x2, c], [x2, 0, c], [c, 0, x2]][i].map(v => v + m);
}

function spreadIter() {
  tmp.set(sim);
  for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) {
    const i = I(x, y), a = sim[i + 3];
    if (a < 0.006) continue;
    const r = sim[i], g = sim[i + 1], b = sim[i + 2];
    const ns = [];
    ...
    for (const di of ns) {
      const ex = a - sim[di + 3]; if (ex < 0.004) continue;
      const f = Math.min(ex * 0.38 / ns.length, a * 0.16); if (f < 1e-4) continue;
      const oa = tmp[di + 3], na = Math.min(1.0, oa + f), act = na - oa; if (act < 1e-4) continue;
...
function gravityIter(rate) {
  for (let y = H - 2; y >= 0; y--) for (let x = 0; x < W; x++) {
    const i = I(x, y), a = sim[i + 3]; if (a < 1e-4) continue;
    const bi = I(x, y + 1), ba = sim[bi + 3], space = 1.0 - ba; if (space < 1e-3) continue;
    const boost = 1.0 + Math.max(0, a - .06) * 9.0;
    const f = Math.min(a * rate * boost * drip[x], a * 0.88, space); if (f < 1e-4) continue;
    const r = sim[i], g = sim[i + 1], b = sim[i + 2], nba = ba + f;
  ...
  for (let x = 0; x < W; x++) { const i = I(x, H - 1); sim[i + 3] = Math.max(0, sim[i + 3] - rate * 5.0); }
  const decay = rate * 0.016;
  for (let k = 0; k < sim.length; k += 4) {
    sim[k + 3] -= decay;
    if (sim[k + 3] < 0.012) { sim[k] = 0; sim[k + 1] = 0; sim[k + 2] = 0; sim[k + 3] = 0; }
  }
}
...
function gravityIterPerPixel(rate) {
  ...
  for (let x = 0; x < W; x++) {
    ...
    const i = I(x, H - 1); sim[i + 3] = Math.max(0, sim[i + 3] - rate * 5.0);
  }
  const decay = rate * 0.016;
  for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) {
...
function splat(nx, ny) {
  ...
  hue = (hue + .14 + Math.random() * .14) % 1;
  const [pr, pg, pb] = hsl(hue, .88 + Math.random() * .12, .36 + Math.random() * .24);
  ...
  for (let y = Math.max(0, (sy - scan) | 0); y < Math.min(H, (sy + scan + 1) | 0); y++)
    for (let x = Math.max(0, (sx - scan) | 0); x < Math.min(W, (sx + scan + 1) | 0); x++) {
      ...
      const k = I(x, y);
      sim[k] = pr; sim[k + 1] = pg; sim[k + 2] = pb;
...
function uploadTexture(dataTexture) {
  ...
  for (let i = 0; i < waitMap.length; i++) {
    if (waitMap[i] > 0) { waitMap[i]--; anyWaiting = true; }
  }
  ...
  for (let y = 0; y < H; y++) {
    const sy = y * W * 4, dy = (H - 1 - y) * W * 4;
    for (let x = 0; x < W; x++) {
      const s = sy + x * 4, d = dy + x * 4;
      const a = sim[s + 3];
      if (a < 0.012) { data[d] = 0; data[d + 1] = 0; data[d + 2] = 0; data[d + 3] = 0; }
      else {
        data[d] = (sim[s] * a * 255 + .5) | 0;
        data[d + 1] = (sim[s + 1] * a * 255 + .5) | 0;
        data[d + 2] = (sim[s + 2] * a * 255 + .5) | 0;
        data[d + 3] = (a * 255 + .5) | 0;
      }
    }
  }
...
export { createPaintOverlay, resizePaintOverlay, uploadTexture, splat, clearSim, splashParams, W, H };
